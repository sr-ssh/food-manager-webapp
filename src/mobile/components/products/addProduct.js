...
export const AddProduct = (props) => {
  ...
  let handleChange = (e) => {
    if ((e.target.id === "price" || e.target.id === "supply" || e.target.id === "discount") && e.target.value.length)
    ...
    >
      <Modal.Body className="add-product px-3 add-discount">
        <Button
          ...
          type="button"
          onClick={(e) => {setItem(-1);props.onHide(false);setProduct({});}}
        >
          <img
          ...
          <Row>
              ...
              <Dropdown onToggle={(e) => setDimStatus(!dimStatus)}>
                ...
                >
                  {productTypes?.map((item, index) => (
                    ...
                    >
                      <Col className="text-end pe-1 order-input">
                        {item.name}
                      </Col>
                    </Dropdown.Item>
                  ))}
            ...
            <Col xs={7} className="order-filter-input ps-0 mx-0">
              <Form.Group controlId="img">
                <Form.Label className="pe-2 fw-normal">url عکس</Form.Label>
                <Form.Control
                  style={{ width: "94%" }}
                  className="order-input h-100 py-3 input-box-shadow"
                  type="text"
                  name="name"
                  defaultValue={addProductLoading ? "" : null}
          ...
          <Row className="mt-0 justify-content-between">
            <Col xs={7} className="order-filter-input ps-0 mx-0">
              <Form.Group controlId="price">
                ...
                <Form.Control
                  style={{ width: "94%" }}
                  className="order-input py-3 h-100 input-box-shadow"
                  type="tel"
                  inputMode="tel"
                  pattern="[0-9 ۰-۹]*"
                  value={addProductLoading ? "" : null}
            ...
            <Col xs={5} className="order-filter-input px-0 mx-0">
              <Form.Group controlId="discount" className="ms-2">
          ...
          <Row>
            <Col>
              {addProductLoading ? (
                ...
                <Button
                  className="product-submit border-0 w-100 mt-3 fs-6 py-2"
                  size="lg"
                  type="submit"
                  block
                >
